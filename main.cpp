...
int main(){
ListFilms historyFilms;
Administrator admin;
readFilms("history.txt",historyFilms);
historyFilms.printListFilms();
ListUsers lista;
//readUsers("users.txt",lista,historyFilms,admin);
//lista.printListUsers();
//lista.front().printUser();
ListFilms listF;
int key=0;
key=readFilms("films.txt",listF);
cout<<key<<endl;
listF.printListFilms();
listF.printAllInfo("The Last Song");
