...
public:
	...
	ListaNizom(ListaNizom<T>&& lista);
	const ListaNizom<T>& operator=(const ListaNizom<T>&);
	const ListaNizom<T>& operator=(ListaNizom<T>&&);
	bool prazna() const;
	bool puna() const;
...
template<typename T>
ListaNizom<T>::ListaNizom(ListaNizom<T>&& lista){
vel=lista.velicina();
maxVel=lista.maxVelicina();
elementi=lista.elementi;
lista.elementi=nullptr;
lista.vel=0;
lista.maxVel=0;
}
...
template<typename T>
const ListaNizom<T>& ListaNizom<T>::operator=(ListaNizom<T>&& lista){
delete [] elementi;
vel=lista.velicina();
maxVel=lista.maxVelicina();
elementi=lista.elementi;
...
ListaNizom<T>& ListaNizom<T>::dodajNaLokaciju(int lokacija, const T& objdodaj)
{
	...
	{
		if (puna())
			throw string("Lista je puna. Nije moguce dodavati jos jedan element. ");
		else
		{
